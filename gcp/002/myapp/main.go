...
func simMetrics(h *handler) {
	for {
		r := rand.Intn(4)
		now := time.Now()
		time.Sleep(time.Duration(r) * time.Second)
		h.metrics.duration.With(prometheus.Labels{"path": "/api/simulate"}).Observe(time.Since(now).Seconds())
		// Test metric
		h.metrics.testDuration.With(prometheus.Labels{"path": "/api/simulate"}).Observe(time.Since(now).Seconds())
	}
}
